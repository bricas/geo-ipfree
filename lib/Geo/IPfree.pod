...
An invalid IP address with octets outside the 0-255 range (e.g. "256.1.2.3")
will cause the method to C<croak>.
...
Make the LookUp() faster, which is good if you're going to be calling LookUp()
many times. This will load the entire DB into memory and read from there,
not from disk (good for slow disk or network disks), but use more memory.

Note that if you call LookUp() many times, you'll end up using a lot of memory
anyway, so you'll be better off using a lot of memory from the start by calling
Faster(), and getting an improvement for all calls.
...
Note that the cache uses /25 granularity: IPs 0-127 in the last octet share
one cache entry, and IPs 128-255 share another. For example, 192.168.0.1 and
192.168.0.100 share the same cache entry, but 192.168.0.1 and 192.168.0.200
do not.
