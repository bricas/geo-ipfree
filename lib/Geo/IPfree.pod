...
The database is built from the Regional Internet Registry (RIR) delegation
files. See the C<misc/> directory for conversion tools.
...
Note that LookUp normalizes the last octet to a /25 boundary for cache keys:
IPs 192.168.0.1 and 192.168.0.100 share the same cache key (192.168.0.0),
while 192.168.0.200 uses a different key (192.168.0.128).
...
The database can be regenerated from RIR delegation files using the tools
in the C<misc/> directory.
...
=item * L<https://www.nro.net/about/rirs/statistics/> - RIR delegation statistics
